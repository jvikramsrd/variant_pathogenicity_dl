...
**Figure 5.** All 16 grid cells and the four feature-family ablations, ranked by mean AUROC over the scoreable genes, against the curated-features-only baseline (blue line). The two ablations that remove structural or domain features land among the strongest cells; the two that remove external prior scores land at the bottom, below every cell that keeps them. No configuration reaches the baseline.
