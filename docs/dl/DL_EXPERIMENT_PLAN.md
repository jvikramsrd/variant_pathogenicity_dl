...
Generated by `vpdl-dl results` (Phase 11 of the runbook) — never transcribed.

| paper element | file | source block |
|---|---|---|
| Table 1 — dataset composition and QC | `results/dl/tables.md` (from `canonical.report.json`) | Phase 2 |
| Table 2 — arms, mean ± SD over seeds | `arms.csv` (`gene == mean:scoreable`) | R, S, B, W, M, F, T, P |
| Table 3 — per held-out gene with cohort sizes | `arms.csv` / `cells.csv` | all blocks |
| Table 4 — paired Δ vs the reference arm, with CI | `paired_*.csv` | R (existing models), P (vs P0) |
| Table 5 — pretraining arms | `pretraining.csv` | P |
| Table 6 — calibration per gene | `calibration.csv` | calibration step |
| Table 7 — independent functional validation | `functional.csv` | functional step |
| Figure — ΔAUC forest plot | `paired_*.png` | any comparison |
| Figure — reliability diagrams | `runs/dl/*/reliability_*.png` | calibration step |
| Methods — dataset identity, splits, code version | `cells.csv`, `environment.json`, `MANIFEST.json` | every run |
| Limitations — leakage and homology | `DATA_LEAKAGE_REPORT.md`, `runs/dl/leakage/*` | Phase 2 |
